...
"##############################################################################################
## Configure Exchange 2013 prerequisites on Windows 2012/2012 R2 Server                      ##
...
## Checking FQDN
        ...
        If ( $ipProperties.DomainName -eq "" )
        {
            ...
            "Do you want to update it? Yes (Y) / No (N)"
            $updateFQDN = Read-Host
            If ( $updateFQDN -Eq "Yes" -Or $updateFQDN -Eq "Y" -Or $updateFQDN -Eq "yes" -Or $updateFQDN -Eq "y")
            {
                "Enter the domain name to use: "
                $DomainName = Read-Host
                Set $ipProperties.DomainName = $DomainName
            }
            If ( $updateFQDN -Eq "No" -Or $updateFQDN -Eq "N" -Or $updateFQDN -Eq "no" -Or $updateFQDN -Eq "n")
            {
                Write-Host -f Red "Existing"
                Break
            }
